...
## Use Case 2: Trace Linked Concepts from a Query Result

**Scenario:** After finding the "Circuit Breaker" concept above, the developer wants to understand what else is connected — what led to this decision, what depends on it, and what related patterns exist.

**Input:**

```bash
# First, see what links to/from this concept
okf links decisions/circuit-breaker-adoption

# Then explore the neighborhood (depth 2)
okf links decisions/circuit-breaker-adoption --depth 2
```
...
{
  "inbound": [
    "bugs/timeout-cascade-fix",
    "patterns/retry-with-backoff"
  ],
  "outbound": [
    "architecture/external-api-layer",
    "patterns/bulkhead-isolation"
  ]
}
```

**Expected Output (depth 2, neighborhood):**
...
```json
{
  "neighborhood": [
    ...
    "architecture/external-api-layer",
    "patterns/bulkhead-isolation",
    "decisions/api-gateway-selection",
    "architecture/service-mesh-overview"
  ]
}
```

**Follow-up — load a specific concept:**

```bash
okf show architecture/external-api-layer
```
...
type: Architecture
tags: apis, resilience, networking
bundle: team

All external API calls go through a dedicated adapter layer (src/adapters/).
Each adapter implements retry, circuit breaker, and timeout policies.
Key decision: adapters own their own config, not a central policy file.
See [Circuit Breaker](decisions/circuit-breaker-adoption.md) for rationale.
```

**Why it's useful:** The link graph turns isolated knowledge into a navigable web. A developer can start with one concept and trace outward to build a mental model of how decisions, patterns, and architecture relate — without reading the entire bundle.

---

## Use Case 3: Understand an Unfamiliar Subsystem Quickly
...
## Use Case 4: Agent-Assisted Discovery (Automated Workflow)
...
**Agent workflow (automated via steering + hooks):**

> *Steering files* instruct the agent on how to use okf-tools (what commands to run, when). *Hooks* trigger actions automatically at specific IDE events — e.g. "run `okf fetch` when a new prompt is submitted" or "run `okf lint` after a task completes."

```bash
# 1. Agent fetches context (triggered by promptSubmit hook)
...
# 5. Post-task lint (triggered by postTaskExecution hook)
okf lint
```
...
## Use Case 5: Quick Bundle Health Check

**Scenario:** A tech lead wants to understand how well the team bundle is being maintained — are concepts being committed, is the link graph connected, are there stale orphans?
...
**Why it's useful:** At a glance, the lead can see coverage gaps (no "testing" concepts?), maintenance needs (6 orphans to connect or remove), and activity trends. This feeds into curation decisions and can be reported as a proof-point metric.
...
| Trace relationships | `okf links <id> --depth N` | ~1 second |
